      ...
      <div className='calenderContainer'>
        ...
        className='progressBox'>
          <div className='progress_bar'> {/*컴포넌트 바 임의 수정*/}
            ...
            { /*테스트용 진행도바*/ }
            <ProgressBar now={calculateProgress(selectedPlan)} label={`${calculateProgress(selectedPlan)}%`} />   {/*성취율 값 넣기 (now,lable)*/}
            

          </div>
          <div className='progress_list'>
    {
      selectedPlan && selectedPlan.planList ? (
        <ul>
          {selectedPlan.planList.map((item, i) => (
            <li key={i} className="task-card" data-index={i % 9}>
              <span>🔥{i + 1}. {item}</span>
            </li>
          ))}
        </ul>
